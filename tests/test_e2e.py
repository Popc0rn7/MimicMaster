...
pytestmark = pytest.mark.skipif(
    os.getenv("RUN_E2E") != "1",
    reason="Set RUN_E2E=1 to run tests against real external services.",
)
...
async def test_service_connectivity():
    ...
    if not embedding_provider_configured():
        pytest.skip("Real embedding provider is not configured.")
    if settings.use_mock_reranker:
        pytest.skip(
            "Mock reranker mode enabled. Set PROVIDER_BASE_URL to use real service."
        )
    ...
    assert (
        len(embed_response.embeddings[0].dense) == settings.embedding_dimension
    ), f"Dense dimension should be {settings.embedding_dimension}"
    assert isinstance(
        embed_response.embeddings[0].sparse.indices, list
    ), "Sparse indices should be a list"
    assert isinstance(
        embed_response.embeddings[0].sparse.values, list
    ), "Sparse values should be a list"

    # Test reranker service
    reranker_service = get_reranker_service()
    rerank_response = await reranker_service.rerank(
        query="test", documents=["test document"], top_n=1
    )
...
async def test_reranking():
    ...
    assert len(response_all.results) <= len(
        documents
    ), "Should not return more than input documents"

    # Verify scores are in descending order
    for i in range(len(response_all.scores) - 1):
        assert (
            response_all.scores[i] >= response_all.scores[i + 1]
...
async def test_ingest_pipeline():
    ...
    assert len(embed_response.embeddings) == len(
        ids
    ), "Embedding count should match input"
...
async def test_complete_pipeline():
    ...
    assert len(rerank_response.results) == len(
        initial_results
    ...
    for i in range(len(rerank_response.scores) - 1):
        assert (
            rerank_response.scores[i] >= rerank_response.scores[i + 1]
