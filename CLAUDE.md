...
- Embedding 默认：NVIDIA NIM `baai/bge-m3` API；本地/内网仅保留 HTTP provider 扩展点
...
3. 依赖管理：尽量保持依赖精简。核心依赖为 Pinecone、OpenAI-compatible SDK、FastAPI；不要在本仓库下载或加载 embedding 模型。
