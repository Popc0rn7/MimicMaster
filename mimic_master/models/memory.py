...
class CharacterState(BaseModel):
    ...
    spell_slots: Dict[int, int] = Field(
        default_factory=dict, description="Spell slots by level"
    )
...
class Episode(BaseModel):
    ...
    key_events: List[str] = Field(
        default_factory=list, description="Key events in episode"
    )
...
class Intent(BaseModel):
    ...
    type: str = Field(
        ..., description="Intent type: QUERY_RULES, PROCEED_STORY, CHAT, COMBAT, etc."
    )
...
class RetrievalConfig(BaseModel):
    ...
    episodes_namespace: str = Field(
        default="episodes", description="Namespace for episodes"
    )
    monsters_namespace: str = Field(
        default="monsters", description="Namespace for monsters"
    )
...
class KnowledgeMetadata(BaseModel):
    ...
    monster_type: str = Field(
        default="", description="Monster type (aberration, dragon, etc.)"
    )
    size: str = Field(
        default="", description="Size (tiny, small, medium, large, huge, gargantuan)"
    )
    ...
    rule_type: str = Field(
        default="",
        description="Rule type: class, spell, race, background, feat, skill, equipment",
    )
    section: str = Field(default="", description="Section name")
    # For DM guides
    dm_type: str = Field(
        default="", description="DM type: treasure, trap, encounter, variant"
    )
    # Image
    has_image: bool = Field(
        default=False, description="Whether entry has associated image"
    )
